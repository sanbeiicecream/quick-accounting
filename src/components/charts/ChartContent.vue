...
export default class ChartContent extends Vue {
  ...
  getChartDate() {
    ...
    if (this.$store.state.chartOrderByTime === '年') {
      ...
      let groupMonthRecordList = getGroupMonthRecordList(this.$store.state.recordListHash[dayjs().get("month")].recordList)
      let groupPreviousMonthRecordList = getGroupMonthRecordList(this.$store.state.recordListHash[dayjs().subtract(1,'month').get("month")].recordList)
      if (this.$store.state.chartPayOrIncome === '支出') {
        this.chartData = getWeekOrMonthChartDate('周',[...groupPreviousMonthRecordList,...groupMonthRecordList],"-")
      } else {
        this.chartData = getWeekOrMonthChartDate('周',[...groupPreviousMonthRecordList,...groupMonthRecordList],"+")
      }
      ...
      let groupMonthRecordList = getGroupMonthRecordList(this.$store.state.recordListHash[dayjs().get("month")].recordList)
      if (this.$store.state.chartPayOrIncome === '支出') {
        this.chartData = getWeekOrMonthChartDate('月',groupMonthRecordList,"-")
      } else {
        this.chartData = getWeekOrMonthChartDate('月',groupMonthRecordList,"+")
      }
    }
